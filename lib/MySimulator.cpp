...
void UpdateDirection(struct PersonStatus &p_stat, struct myBoundary &my_bound) {
  ...
  if (p_stat.m_coordinate.second < my_bound.down_y) {
    p_stat.m_coordinate.second =
        2 * my_bound.down_y - p_stat.m_coordinate.second;
...
void RecoveredOrDead(SimulationState &status) {
  ...
  for (auto i = status.g_infected_people.begin();
       i != status.g_infected_people.end(); ++i) {
    ...
    if (recovered_prob <= i->second.m_recovery_rate) {
      ...
      if (i->second.m_quar == 2 || i->second.m_quar == 1) {
        ...
        status.g_person_status[i->second.m_index].rx =
            status.g_boundary.right_x;
  ...
  // erase the Recovered or dead people
  ...
  for (auto iter = status.g_tmp_new_infected_people.begin();
       iter != status.g_tmp_new_infected_people.end(); ++iter) {
    status.g_infected_people.insert({*iter, status.g_person_status[*iter]});
  }
  status.g_tmp_new_infected_people.clear();
}
...
void SpreadVirus(SimulationState &status) {
  ...
  for (auto i = status.g_infected_people.begin();
       i != status.g_infected_people.end(); ++i) {
    for (auto j = status.g_health_people.begin();
         j != status.g_health_people.end(); ++j) {
      ...
      if (dist <= status.m_spread_range) {
        ...
        if (infected_prob <= status.m_infect_rate) {
          ...
          status.g_tmp_new_infected_people.push_back(
              status.g_person_status[*j].m_index);
          j = status.g_health_people.erase(j);
          --j;
        }
      }
    }
...
void CheckPolicy(SimulationState &status) {
  if (status.m_policy == 1) {
    size_t count = 0;
    for (auto iter = status.g_infected_people.begin();
         iter != status.g_infected_people.end() &&
         count < status.m_healthcare_capacity;
         ++iter) {
...
int main() {
  ...
  SimulationState default_data(10000, 50, 1, 1.0, 0.05, 0.05, 100, 0.0, 2.0,
                               2.0, 0.0, 1000, 0, 0.001, 0.02, 0, 0.95);

  size_t count = 1000;
  while (--count) {
    RunStep(default_data);
  }
  return 0;
}
